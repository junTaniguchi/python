...
def gaussianBlur_image(img_name, 
    ...
    img_file_name = "./image/" + place_name + "/" + str(idx1) + img_name + "_GaussianBlur_" + str(font_size) + str(x_pixel) + str(y_pixel) + place_name + '.png'
    # 画像ファイルを保存する
    #blur_image.save(img_file_name, 'PNG')
    img_detail = [idx2, x_pixel, y_pixel, width, height]
                ...
                if limit_x_draw_pixel > 20:
                    ...
                    for x_draw_pixel in x_range:
                        for y_draw_pixel in y_range:
                            ...
                            gaussianBlur_image(img_name, place_name, image_np, idx1, idx2, X, Y, font_size, x_draw_pixel, y_draw_pixel, width, height)
                            print(i)
                            i+=1
    X = np.array(X)
    Y = np.array(Y)
