...
for idx1, jp_font in enumerate(jp_font_list):
    # フォントの指定。引数は順に「フォントのパス」「フォントのサイズ」「エンコード」
    # メソッド名からも分かるようにTure Typeのフォントを指定する
    font = ImageFont.truetype(jp_font, 50)
    ...
    for idx2, place_name in enumerate(place_list):
        ...
        # 日本語の文字を入れてみる
        # 引数は順に「(文字列の左上のx座標, 文字列の左上のy座標)」「フォントの指定」「文字色」
        draw.text((140, 150), place_name, font=font, fill='#ffffff')
...
for idx1, jp_font in enumerate(jp_font_list):
    # フォントの指定。引数は順に「フォントのパス」「フォントのサイズ」「エンコード」
    # メソッド名からも分かるようにTure Typeのフォントを指定する
    font = ImageFont.truetype(jp_font, 40)
    ...
    for idx2, place_name in enumerate(place_list):
        ...
        # 日本語の文字を入れてみる
        # 引数は順に「(文字列の左上のx座標, 文字列の左上のy座標)」「フォントの指定」「文字色」
        draw.text((100, 150), place_name, font=font, fill='#ffffff')
        # opencvのガウシアンフィルターを適応
        image_np = np.asarray(image)
        blur = cv2.GaussianBlur(image_np, (5, 5), 0)
        # 画像の中心位置
        ...
        # 画像ファイルを保存する
        affine_image.save("./image/" + place_name + "/" + str(idx1) + "_GaussianBlur_Affine45_" + place_name + '.png', 'PNG')
...
for idx1, jp_font in enumerate(jp_font_list):
    # フォントの指定。引数は順に「フォントのパス」「フォントのサイズ」「エンコード」
    # メソッド名からも分かるようにTure Typeのフォントを指定する
    font = ImageFont.truetype(jp_font, 30)
    ...
    for idx2, place_name in enumerate(place_list):
        ...
        # 日本語の文字を入れてみる
        # 引数は順に「(文字列の左上のx座標, 文字列の左上のy座標)」「フォントの指定」「文字色」
        draw.text((150, 150), place_name, font=font, fill='#ffffff')
        # opencvのガウシアンフィルターを適応
        image_np = np.asarray(image)
        blur = cv2.GaussianBlur(image_np, (5, 5), 0)
        # 画像の中心位置
        ...
        # 画像ファイルを保存する
        affine_image.save("./image/" + place_name + "/" + str(idx1) + "_GaussianBlur_Affine320_" + place_name + '.png', 'PNG')
        data = np.asarray(affine_image)        
        X.append(data.astype(np.float64))
        Y.append(idx2)
